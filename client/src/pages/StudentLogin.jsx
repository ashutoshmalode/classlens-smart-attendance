...
import { loginStudent } from '../store/slices/authSlice.js';
import { User, Mail, LogIn, SwitchCamera } from 'lucide-react';

export const StudentLogin = ({ onSwitchToFaculty }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    dispatch(loginStudent({ studentId, email }));
  };
  ...
  return (
    <div className="min-h-screen bg-dark-bg flex items-center justify-center p-6 relative">
      ...
      <div className="w-full max-w-md glass p-8 rounded-3xl border border-white/5 relative z-10 flex flex-col items-center">
        ...
        <h2 className="text-2xl font-extrabold text-white mb-1">Student Lookup</h2>
        <p className="text-gray-400 text-xs mb-8">Access Your Personal Attendance History</p>
        ...
        >
          ...
          <span>Switch to Faculty Login</span>
        </button>
      </div>
    </div>
  );
